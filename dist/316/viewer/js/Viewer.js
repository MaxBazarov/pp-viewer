class StageDiv
{
    ...
    elDiv(elType = "div")
    {
        ...
        if (this.bottom != null) el.style.bottom = this.bottom;

        if (elType === "video")
        {
            if (this.w != null) el.setAttribute("width", this.w);
            if (this.h != null) el.setAttribute("height", this.h);
        } else
        {
            if (this.w != null) el.style.width = this.w;
            if (this.h != null) el.style.height = this.h;
        }
...
function handleDecreaseVersion()
{
    ...
    if ((currentVerIndex + 1) === data['recs'].length) return showMessage(`The current #${story.docVersion} version is first version`)
    //
...
function handleIncreaseVersion()
{
    ...
    const currentVerIndex = viewer._findCurrentVersionIndex(data)
    if (currentVerIndex === null) return
    if (currentVerIndex === 0) return showMessage(`The current #${story.docVersion} version is first version`)
    //
    viewer.infoViewer.goToVersionByRec(data['recs'][currentVerIndex - 1])
}
...
class Viewer
{
    ...
    {
        url = "https://www.figma.com/design/a2v0XsXvEEUhzDnURaQxmg/%25D0%2593%25D0%25BB%25D0%25B0%25D0%25B2%25D0%25BD%25D1%258B%25D0%25B5-%25D1%2581%25D1%2582%25D1%2580%25D0%25B0%25D0%25BD%25D0%25B8%25D1%2586%25D1%258B?node-id=9062-8394&t=AGFfLFDQon2FrJtY-11"
    ...
    _parseLocationSearch()
    {
        ...
        {
            const frameIDSrc = this.urlParams.get("frameID")
            if (frameIDSrc)
            {
                const frameID = frameIDSrc.replace("-", ":");
                if (frameID)
                {
                    const foundPages = story.pages.filter(p => p.id === frameID);
                    if (foundPages.length)
                    {
                        result.page_name = foundPages[0].index;
                    }
