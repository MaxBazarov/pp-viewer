...
///
const Constants = {
    ARTBOARD_OVERLAY_PIN_HOTSPOT: "HOTSPOT",
    ARTBOARD_OVERLAY_PIN_PAGE: "PAGE",
    //
    ARTBOARD_OVERLAY_PIN_HOTSPOT_UNDER_LEFT: "UNDER_LEFT",
    ARTBOARD_OVERLAY_PIN_HOTSPOT_UNDER_CENTER: "UNDER_CENTER",
    ARTBOARD_OVERLAY_PIN_HOTSPOT_UNDER_RIGHT: "UNDER_RIGHT",
    ARTBOARD_OVERLAY_PIN_HOTSPOT_TOP_LEFT: "TOP_LEFT",
    ARTBOARD_OVERLAY_PIN_HOTSPOT_TOP_CENTER: "TOP_CENTER",
    ARTBOARD_OVERLAY_PIN_HOTSPOT_TOP_RIGHT: "TOP_RIGHT",
    ARTBOARD_OVERLAY_PIN_HOTSPOT_BOTTOM_RIGHT: "BOTTOM_RIGHT",
    ARTBOARD_OVERLAY_PIN_HOTSPOT_UP_CENTER: "UP_CENTER",
    ARTBOARD_OVERLAY_PIN_HOTSPOT_RELATIVE: "RELATIVE",
    //
    ARTBOARD_OVERLAY_PIN_PAGE_TOP_LEFT: "TOP_LEFT",
    ARTBOARD_OVERLAY_PIN_PAGE_TOP_CENTER: "TOP_CENTER",
    ARTBOARD_OVERLAY_PIN_PAGE_TOP_RIGHT: "TOP_RIGHT",
    ARTBOARD_OVERLAY_PIN_PAGE_BOTTOM_LEFT: "BOTTOM_LEFT",
    ARTBOARD_OVERLAY_PIN_PAGE_BOTTOM_CENTER: "BOTTOM_CENTER",
...
class ViewerPage
{
    ...
    _createVideoPlayers()
    {
        this.videoNodes.forEach((n) =>
        {
            ...
            video.setAttribute("src", "images/" + n.n);
        ...
        if (!this.parentPage || this.parentPage.id != newParentPage.id || hasClass(div, 'hidden'))
        {
            ...
            } else
            {
                ...
                if (!this.overlayClosePrevOverlay && !positionCloned && undefined != this.overlayShadowX &&
                    (
                        (Constants.ARTBOARD_OVERLAY_PIN_HOTSPOT == reaction.overlayPinType) // ARTBOARD_OVERLAY_PIN_HOTSPOT
                        && (3 == reaction.overlayPinHotspot) //ARTBOARD_OVERLAY_PIN_HOTSPOT_TOP_LEFT
                    )
                )
    ...
    if (reaction.frameIndex != undefined)
    {
        ...
        } else if ('overlay' == destPage.type)
        {
            ...
            { // clicked not from fixed panel           
                const pinHotspot = Constants.ARTBOARD_OVERLAY_PIN_HOTSPOT == reaction.overlayPinType
                const pinPage = Constants.ARTBOARD_OVERLAY_PIN_PAGE == reaction.overlayPinType
                ...
                if (pinHotspot)
                {
                    ...
                    var offsetX = pinHotspot
                        && (
                            Constants.ARTBOARD_OVERLAY_PIN_HOTSPOT_UNDER_LEFT == reaction.overlayPinHotspot
                            || Constants.ARTBOARD_OVERLAY_PIN_HOTSPOT_UNDER_CENTER == reaction.overlayPinHotspot
                            || Constants.ARTBOARD_OVERLAY_PIN_HOTSPOT_UNDER_RIGHT == reaction.overlayPinHotspot
                        )
                        ? 5 : 0
                    ...
                    } else if (pinHotspot && Constants.ARTBOARD_OVERLAY_PIN_HOTSPOT_UNDER_LEFT == reaction.overlayPinHotspot)
                    {
                        pageY += link.rect.height
                    } else if (pinHotspot && Constants.ARTBOARD_OVERLAY_PIN_HOTSPOT_UNDER_CENTER == reaction.overlayPinHotspot)
                    {
                        pageX += parseInt(orgLink.width / 2) - parseInt(destPage.width / 2)
                        pageY += link.rect.height
                    } else if (pinHotspot && Constants.ARTBOARD_OVERLAY_PIN_HOTSPOT_UNDER_RIGHT == reaction.overlayPinHotspot)
                    {
                        pageX += orgLink.width - destPage.width
                        pageY += link.rect.height
                    } else if (pinHotspot && Constants.ARTBOARD_OVERLAY_PIN_HOTSPOT_TOP_LEFT == reaction.overlayPinHotspot)
                    {
                    } else if (pinHotspot && Constants.ARTBOARD_OVERLAY_PIN_HOTSPOT_TOP_CENTER == reaction.overlayPinHotspot)
                    {
                        pageX += parseInt(orgLink.width / 2) - parseInt(destPage.width / 2)
                        //pageY -= destPage.height                            
                    } else if (pinHotspot && Constants.ARTBOARD_OVERLAY_PIN_HOTSPOT_TOP_RIGHT == reaction.overlayPinHotspot)
                    {
                        pageX += orgLink.width - destPage.width
                        //pageY = pageY - destPage.height                            
                    } else if (pinHotspot && Constants.ARTBOARD_OVERLAY_PIN_HOTSPOT_BOTTOM_RIGHT == reaction.overlayPinHotspot)
                    {
                        pageX += orgLink.width
                    } else if (pinHotspot && Constants.ARTBOARD_OVERLAY_PIN_HOTSPOT_UP_CENTER == reaction.overlayPinHotspot)
                    {
                        pageX += parseInt(orgLink.width / 2) - parseInt(destPage.width / 2)
                        pageY -= destPage.height
                    } else
                    {
                        if (reaction.overlayPinHotspot === Constants.ARTBOARD_OVERLAY_PIN_HOTSPOT_RELATIVE)
                        {
                    ...
                    if (!pinHotspot || (Constants.ARTBOARD_OVERLAY_PIN_HOTSPOT_TOP_LEFT != reaction.overlayPinHotspot))
                    {
