...
const GALLERY_FRAME_LABEL_HEIGHT = 12
...
class GalleryViewer extends AbstractViewer
{
    ...
    positionContent()
    {
        ...
        canvas.width = this.currentFullWidth * 2
        canvas.height = this._zoomPos(this.fullHeight)

        //
        story.groups.forEach(function (group)
        {
            this.positionContent_Group(ctx, group)
            group.pages.forEach(page => this.positionContent_Page(ctx, page))
        }, this);
        //this._buildLinks()
    }

    positionContent_Group(ctx, group)
    {
        const x = 0
        const y = this._zoomPos(group.globTop)
        const width = this._zoomPos(this.maxGroupWidth)
        const height = this._zoomPos(group.height)

        ctx.fillStyle = story.galleryPageColorsEnabled ? group.backColor : ""
        ctx.fillRect(x, y, width, height)
    }

    positionContent_Page(ctx, page)
    {
        // position frame label
        if (page.isFrame && this.zoom >= GALLERY_FRAME_LABEL_MINZOOM)
        {
            const top = page.globalTop - (this.zoom < 0.5 ? (GALLERY_FRAME_LABEL_HEIGHT / this.zoom * 0.6) : GALLERY_FRAME_LABEL_HEIGHT)
            ctx.font = "24px Tahoma"
            ctx.fillStyle = invertColor(page.group.backColor)
            ctx.fillText(page.title, this._zoomPos(page.globalLeft, GALLERY_LEFTRIGH_MARGIN), this._zoomPos(top));
        }
        // position frame itself
        {
            const left = this._zoomPos(page.globalLeft, GALLERY_LEFTRIGH_MARGIN)
            const top = this._zoomPos(page.globalTop)
            const width = this._zoomPos(page.width)
            const height = this._zoomPos(page.height)

            ctx.drawImage(page.imageObj[0], left, top, width, height)
        }
        /// position image
    ...
    {
        return Math.round((pos * this.zoom + absDelta) * 2)
    }
