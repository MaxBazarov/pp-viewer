...
class ViewerPage
{
    ...
    showAsOverlayInCurrentPage(orgPage, link, posX = 0, posY = 0, linkParentFixed = false, disableAnim = false)
    {
        ...
        if (!this.parentPage || this.parentPage.id != newParentPage.id || div.hasClass('hidden'))
        {
            ...
            if (link.fixedBottomPanel)
            {
                ...
                if (link.fixedBottomPanel === true)
                {
                    posX = (newParentPage.width - this.width) / 2
                    posY = 0
                } else
                {
    ...
    _loadSingleImage(sizeSrc, idPrefix)
    {
        ...
        var img = $('<img/>', {
            ...
            class: "pageImage",
            src: encodeURIComponent(viewer.files) + '/' + (sizeSrc.imageFixedLess ? sizeSrc.imageFixedLess : encodeURIComponent(sizeSrc.image)) + unCachePostfix,
...
function handleLinkEvent_Final(link, orgPage)
{
    ...
    if (link.name != "" && orgPage)
    {
        ...
        {
            if (p.overlayPinPage === Constants.ARTBOARD_OVERLAY_PIN_PAGE_BOTTOM_CENTER) link.fixedBottomPanel = true
